'''Faça um Programa que pergunte quanto você ganha por hora e o número de horas trabalhadas no mês. 
Calcule e mostre o total do seu salário no referido mês, sabendo-se que são descontados 11% para o Imposto de Renda, 8% para o INSS e 5% para o sindicato, faça um programa que nos dê:
salário bruto.
quanto pagou ao INSS.
quanto pagou ao sindicato.
o salário líquido.
calcule os descontos e o salário líquido, conforme a tabela abaixo:
+ Salário Bruto : R$
- IR (11%) : R$
- INSS (8%) : R$
- Sindicato ( 5%) : R$
= Salário Liquido : R$'''

ganha_hora = float(input('Digite quanto você ganha por hora trabalhada: R$ '))
hora_trabalhada = int(input('Digite quantas horas você trabalhou esse mês: '))

salario_bruto = ganha_hora * hora_trabalhada
ir = salario_bruto * 0.11
inss = salario_bruto * 0.08
sindicato = salario_bruto * 0.05
salario_liquido = salario_bruto - ir - inss - sindicato

print(f'Seu salário bruto este mês é de: R$ {salario_bruto:.2f}')
print(f'Sua contribuição ao INSS este mês é de: R$ {inss:.2f}')
print(f'Sua contribuição ao sindicato este mês é de: R$ {sindicato:.2f}')
print(f'Seu salário líquido este mês é de: R$ {salario_liquido:.2f}')
